...
export default function Projects() {
  ...
  return (
    <section id="projects" className="min-h-screen flex flex-col justify-center py-24 md:py-32 border-t border-gray-200/50 relative">
          ...
          <div className="md:hidden flex flex-col gap-8">
            {projectsData.map((project) => (
              <div 
                ...
                className="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.08)] overflow-hidden active:scale-[0.98] transition-transform"
              >
                <div className="relative w-full h-56 bg-slate-100">
                  <Image src={project.image} alt={project.title} fill className="object-cover" />
                  <span className="absolute top-4 left-4 bg-white/95 backdrop-blur-md px-4 py-1.5 rounded-full text-[10px] font-extrabold text-blue-600 uppercase tracking-widest shadow-sm">
                    {project.tags[0]}
                  </span>
                </div>
            ...
            {projectsData.map((project) => (
              ...
              >
                ...
                <div className="p-8 flex flex-col flex-grow relative z-10">
                    ...
                    <p className="text-gray-600 mb-2 flex-grow leading-relaxed text-sm">
                      {project.shortDescription}
                    </p>
                </div>
              </div>
      ...
      {activeProject && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-gray-900/90 md:bg-gray-900/80 backdrop-blur-md md:p-10" onClick={closeModal}>
          ...
          <div className="md:hidden w-full h-full bg-white flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
            
            {/* FIXED HEADER FOR MOBILE: Hindi na kailanman matatakpan ng kahit ano */}
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100 bg-white flex-shrink-0">
              <h3 className="font-extrabold text-lg text-gray-900 truncate max-w-[75%]">
                {activeProject.title}
              </h3>
              <button 
                onClick={closeModal} 
                className="bg-gray-100 hover:bg-gray-200 text-gray-900 w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm active:scale-90 transition-transform"
              >
                ✕
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="relative w-full h-64 bg-slate-50 border-b border-gray-100">
                ...
                {displayImages.length > 1 && (
                  <>
                    <button onClick={prevImage} className="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center bg-white/70 text-gray-900 rounded-full shadow-md">
                      <FaChevronLeft className="pr-0.5 text-xs" />
                    </button>
                    <button onClick={nextImage} className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center bg-white/70 text-gray-900 rounded-full shadow-md">
                      <FaChevronRight className="pl-0.5 text-xs" />
                    </button>
                    <div className="absolute bottom-4 w-full flex justify-center gap-1.5">
                      {displayImages.map((_, idx) => (
                        <span key={idx} className={`h-1.5 rounded-full transition-all ${idx === currentImageIndex ? "w-6 bg-blue-600" : "w-1.5 bg-gray-300"}`}></span>
                      ))}
                    </div>
                  </>
                )}
              ...
              <div className="p-6 pb-24">
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {activeProject.tags.map((tag) => (
                    <span key={tag} className="text-[10px] font-bold px-3 py-1 bg-gray-100 text-gray-600 rounded-full">
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                <h4 className="text-xs font-bold tracking-widest uppercase text-gray-400 mb-2">Project Overview</h4>
                <p className="text-gray-600 text-sm leading-relaxed mb-6">{activeProject.fullDescription}</p>

                <h4 className="text-xs font-bold tracking-widest uppercase text-gray-400 mb-3 flex items-center gap-2">
                  <FaCode className="text-blue-500" /> Technologies Used
                </h4>
                <div className="flex flex-wrap gap-2">
                  {activeProject.techStack.map((tech) => (
                    <span key={tech} className="px-3 py-1.5 bg-blue-50/50 border border-blue-100/50 text-blue-700 text-xs font-semibold rounded-lg">
                      {tech}
                    </span>
                  ))}
                </div>
          ...
          <div className="hidden md:flex relative w-full max-w-6xl max-h-[90vh] bg-white rounded-3xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <button onClick={closeModal} className="absolute top-4 right-4 z-50 bg-white/80 backdrop-blur-md hover:bg-red-500 text-gray-900 hover:text-white w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 shadow-md">
              ✕
            </button>

            {/* Desktop Left: Image Gallery */}
            <div className="w-1/2 h-auto relative bg-slate-100 group/gallery flex-shrink-0">
              ...
              {displayImages.length > 1 && (
                <>
                  <button onClick={prevImage} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center bg-white/80 hover:bg-white text-gray-800 shadow-lg rounded-full transition-all opacity-0 group-hover/gallery:opacity-100">
                    <FaChevronLeft className="pr-0.5" />
                  </button>
                  <button onClick={nextImage} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center bg-white/80 hover:bg-white text-gray-800 shadow-lg rounded-full transition-all opacity-0 group-hover/gallery:opacity-100">
                    <FaChevronRight className="pl-0.5" />
                  </button>
                  <div className="absolute bottom-6 left-0 w-full flex justify-center gap-2">
                    {displayImages.map((_, idx) => (
                      <span key={idx} className={`h-2 rounded-full transition-all duration-300 ${idx === currentImageIndex ? "w-8 bg-blue-600 shadow-md" : "w-2 bg-gray-300/80"}`}></span>
                    ))}
                  </div>
                </>
              )}
            ...
            <div className="w-1/2 flex flex-col h-full overflow-y-auto bg-white p-12 lg:p-14">
              <div className="mb-8">
                <div className="flex flex-wrap gap-2 mb-4">
                  {activeProject.tags.map((tag) => (
                    <span key={tag} className="text-xs font-bold px-3 py-1 bg-gray-100 text-gray-600 rounded-full">{tag}</span>
                  ))}
                </div>
                <h2 className="text-3xl lg:text-4xl font-extrabold text-gray-900 leading-tight mb-4">{activeProject.title}</h2>
                {activeProject.liveUrl && (
              ...
              <div className="mb-10">
                ...
                <p className="text-gray-600 text-base leading-relaxed">{activeProject.fullDescription}</p>
              </div>
              <div className="mt-auto pt-8 border-t border-gray-100">
                ...
                <div className="flex flex-wrap gap-2">
                  {activeProject.techStack.map((tech) => (
                    <span key={tech} className="px-4 py-2 bg-blue-50/50 border border-blue-100/50 text-blue-700 text-sm font-semibold rounded-lg">{tech}</span>
                  ))}
                </div>
              </div>
            </div>
